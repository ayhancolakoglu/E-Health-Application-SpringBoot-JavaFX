...
@Component
@FxmlView("/ui/OtorhinolaryngologistPage.fxml")
public class OtorhinolaryngologistPageController implements Initializable {
    ...
    void loadDoctorDetails() {
        ...
        doctorList.addAll(doctorService.findByMedicalField("Otorhinolaryngologist"));
    ...
    void onSaveAppointment(ActionEvent event) {
        ...
        if (appointmentService.appointmentExists(appointment.getDate(), appointment.getTime(), appointment.getDoctor().getLastname(), appointment.getDoctor().getMedicalField())) {
            ...
            try {
                String filename = appointment.getUser().getUsername() + "_" +
                        appointment.getDoctor().getUsername() + "_" +
                        appointment.getId() + ".txt";


                FileWriter healthinfo = new FileWriter(filename);

                healthinfo.write("Hello Mr/Mrs. " + FxApplication.currentuser.getLastname() + ",\n" +
                        "In the following you can see your appointment information and general information\n" +
                        "Doctor: " + appointment.getDoctor().getTitle() + ". " + appointment.getDoctor().getFirstname() + " " + appointment.getDoctor().getLastname() +
                        "\nAddress: " + appointment.getDoctor().getAddress() + appointment.getDoctor().getZip() + appointment.getDoctor().getCity() + "\n" +
                        "Appointment Information: " + appointment.getDate() + " at " + appointment.getTime() + "\n" +
                        "Appointment health information: " + appointment.getDescription() +
                        "General health information: " + appointment.getUser().getHealthInformation() + " \n"
                );
                healthinfo.close();
                ...
                String filestringname = appointment.getUser().getUsername() + "_" + appointment.getDoctor().getUsername() + "_" + appointment.getId() + ".txt";
